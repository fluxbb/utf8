<?php
/**
 * UTF-8 aware alternative to sprintf and vprintf.
 * 
 * @package php-utf8
...
/**
 * UTF-8 aware alternative to sprintf.
 * 
 * @uses utf8_vprintf
 * @param string $format
 * @return string
...
/**
 * UTF-8 aware alternative to vprintf.
 *
 * @param string $format
...
function utf8_vprintf($format, $arguments)
{
	...
	if( count($results) )
	{
		foreach( $results as $result )
		{
			...
			$format = substr($format, 0, $pos).($size ? str_repeat($filler, $size - strlen($arg)) : '').str_replace('%', '%%', $arg).substr($format, $pos + strlen($string_format));
		}
