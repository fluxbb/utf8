...
class test_utf8_position extends UnitTestCase
{
	...
	function test_singlebyte()
	{
		...
		foreach( $tests as $test ) $this->assertIdentical(utf8_locate_current_chr($test[0], $test[1]), $test[2]);

		$tests = array( );
		$tests[] = array( 'aaживπά우리をあöä', 1, 1 );

		foreach( $tests as $test ) $this->assertIdentical(utf8_locate_next_chr($test[0], $test[1]), $test[2]);
	}
